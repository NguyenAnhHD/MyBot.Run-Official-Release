...
Func CreateLogTab($hWHndLogsOnly = False)
	...
	If IsHWnd($g_hTxtLog) Then
		...
		_GUICtrlRichEdit_SetSel($g_hTxtLog, -1, -1) ; select end (scroll to end)
		_WinAPI_RedrawWindow($g_hTxtLog, 0, 0, $RDW_INVALIDATE) ; redraw RichEdit
	Else
	...
	If IsHWnd($g_hTxtAtkLog) Then
		...
		_GUICtrlRichEdit_SetSel($g_hTxtAtkLog, -1, -1) ; select end (scroll to end)
		_WinAPI_RedrawWindow($g_hTxtAtkLog, 0, 0, $RDW_INVALIDATE) ; redraw RichEdit
	Else
